...
void initColor()
{
	...
	{
		colortab[i].val[0] = (i * 11 + randomColor[0]) % 255;
        colortab[i].val[1] = (i * 5 + randomColor[1]) % 255;
        colortab[i].val[2] = (i * 17 + randomColor[2]) % 255;
	}
}
...
void draw_img()
{
    ...
    for(size_t x=0;x<IMAGE_WIDTH;x++)
    {
        ...
        {
            mapping(x,y,re,im);
            int colorIndex =  (mSetTest(re,im)) % 256;
            cv::Vec3b color;
...
void on_mouse( int event, int x, int y, int flags, void *param  =  NULL)
{
	...
	switch(event)
	{
		...
		case CV_EVENT_MOUSEMOVE:
		{
            ...
            {
                pt4 = cvPoint(x,y);
                cvRectangle(pImg,pt1,pt4,cvScalar(255,255,255),-1);
                //cout << "key move" <<endl;
				if(pt1.x > 0 && pt1.y > 0
					&& pt2.x > 0 && pt2.y > 0
					&& abs(pt2.x - pt1.x) > 5
